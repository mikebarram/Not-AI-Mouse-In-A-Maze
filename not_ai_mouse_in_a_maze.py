...
On Windows, to install dot, install Graphviz and make sure the path to the bin folder that contains
dot.exe is in the PATH environment variable (or include the full path to dot when you call it)
...
# mazes that the mouse fails to solve are saved here. Any mazes in here are reloaded at the start of session.
...
# the bigger the maze, the longer it could be before returning to a visited bit of the maze
FRAMES_BETWEEN_BLURRING_VISITED = (ROWS * COLS)//12
FRAME_DISPLAY_RATE = 5
...
def create_trail_circle_alpha(circle_radius, trail_alpha):
    ...
    # the circle will mostly have opacity trail_alpha but be 1/3 and 2/3 that around the outside
    mask_outer = (x_list[np.newaxis,:])**2 + (y_list[:,np.newaxis])**2 < circle_radius**2
    mask_middle = (x_list[np.newaxis,:])**2 + (y_list[:,np.newaxis])**2 < (circle_radius-4)**2
    mask_inner = (x_list[np.newaxis,:])**2 + (y_list[:,np.newaxis])**2 < (circle_radius-8)**2
...
class Backtracking:
    ...
    def generator(self, maze_cx, maze_cy, grid):
        ...
        if grid[maze_cy - 2, maze_cx] == 0.5 and grid[maze_cy + 2, maze_cx] == 0.5 and grid[maze_cy, maze_cx - 2] == 0.5 and grid[maze_cy, maze_cx + 2] == 0.5:
            pass
        else:
            maze_li = [1, 2, 3, 4]
            while len(maze_li) > 0:
...
class mouse():
    ...
    def __init__(self, screen, visited_by_car_screen, track_distances_screen, track):
        ...
        # could try to set initial direction based on the shape of the maze that's generated but this is fine
        ...
        self.car_icon = CarIcon(self.position_rounded[0],self.position_rounded[1])
    ...
    def drive(self, draw_frame):
        ...
        if draw_frame:
            self.draw_lines_to_track_edge(whiskers)

        self.won = self.check_if_position_wins(self.position[0], self.position[1])
        if self.won:
            self.draw_car_finish_location(GREEN)
            return
        ...
        if is_dead_end:
            # old code from maze v3 that doesn't take into account whether pixels have been visited
            ...
            for ted in track_edge_distances:
                ...
                # this takes into account how many of the pixels have been visited but not alpha for each pixel (summed in ted[3])
                distance_for_angle = max(0, ted[1] * (1 - (ted[3] * CAR_VISITED_PATH_AVOIDANCE_FACTOR) / (255 * ted[1])))
        ...
        track_edge_distance, visited_count, visited_alpha_total, edge_x, edge_y = self.get_track_edge_distance(self.track.track_pixels, self.visited_alpha, self.direction_radians, self.position_rounded[0], self.position_rounded[1], new_steering_radians)
        ...
        if draw_frame:
            ...
            surface_alpha = np.array(self.visited_by_car_screen.get_view('A'), copy=False)
        ...
        if self.stats_info_car["distance"] > TRACK_MAX_DISTANCE_TO_COMPLETE:
            self.crashed = True
            self.draw_car_finish_location(RED)
            return

        self.stats_info_car["frames"] += 1
        self.stats_info_car["distance"] += new_speed
        self.stats_info_car["average speed"] = self.stats_info_car["distance"] // self.stats_info_car["frames"]
        self.stats_info_car["CAR_SPEED_MIN"] = CAR_SPEED_MIN
        self.stats_info_car["CAR_SPEED_MAX"] = CAR_SPEED_MAX
        ...
        if draw_frame:
            self.car_icon_group.update(self.position_rounded[0], self.position_rounded[1], self.direction_radians)
    ...
    def update_visited(position, direction_radians, visited_alpha):
        '''Updates a mouses 2D array to record that the mouse has been in a particular area'''
        ...
        section_to_update = visited_alpha[circle_top_left_x:circle_top_left_x+2*CAR_VISITED_PATH_RADIUS,circle_top_left_y:circle_top_left_y+2*CAR_VISITED_PATH_RADIUS]
    ...
    def fade_visited(visited_alpha):
        '''fades the mouses 2D array of records of where it has been - like a scent fading away'''
    ...
    def check_if_position_wins(position_x, position_y):
        ...
        if -2 < position_x/SQUARE_SIZE-COLS < -1 and -2 < position_y/SQUARE_SIZE-ROWS < -1:
            return True
        else:
            return False
    ...
    def get_track_edge_distances(self):
        '''get the distance of the mouse from the edges of the maze along a defined list of angles from its direction of travel'''
        ...
        for vision_angle_and_weight in CAR_VISION_ANGLES_AND_WEIGHTS:
            track_edge_distance, visited_count, visited_alpha_total, edge_x, edge_y = self.get_track_edge_distance(self.track.track_pixels, self.visited_alpha, self.direction_radians, self.position_rounded[0], self.position_rounded[1], vision_angle_and_weight[0])
    ...
    @staticmethod
    @jit(nopython=True)
    def get_track_edge_distance(track_pixels, visited_alpha, direction_radians, position_rounded_x, position_rounded_y, vision_angle):
        '''get the distance of the mouse from the edges of the maze along a single angle from its direction of travel'''
...
def stats_update(stats_surface, stats_info_car, stats_info_global):
    ...
    for stats in (stats_info_car, stats_info_global):
        for stats_key,stats_value in stats.items():
            img = font.render(stats_key + ': ' + str(round(stats_value)), True,
                    pygame.Color(BLACK),
                    pygame.Color(WHITE))
...
def main():
    ...
    saved_maze_files = [name for name in os.listdir(MAZE_DIRECTORY) if os.path.isfile(os.path.join(MAZE_DIRECTORY, name))]
    ...
    while running:
        ...
        if stats_info_global["Total frames"] > 1000000:
            break
        ...
        if new_track_and_car_needed:
            ...
            try:
                ...
                if car.won:
                    ...
                    stats_info_global["Max successes in a row"] = max(stats_info_global["Max successes in a row"],stats_info_global["Success count"])
                elif car.crashed:
                    stats_info_global["Max successes in a row"] = max(stats_info_global["Max successes in a row"],stats_info_global["Success count"])
            ...
            if saved_maze_counter < saved_maze_count:
                ...
                track.create()
            car = mouse(background, visited_by_car_screen, track_distances_screen, track)
        ...
        if stats_info_global["Total frames"] % FRAME_DISPLAY_RATE == 0 or car.won is True or car.crashed is True:
            draw_frame = True
        else:
            draw_frame = False
